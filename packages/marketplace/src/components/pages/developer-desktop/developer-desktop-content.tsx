...
import { H5, Grid, GridItem, Content, Section } from '@reapit/elements'
import VideoClip from '@/assets/images/developer-desktop/video-clip.jpg'

export const DeveloperDesktopContentPartOne: React.FC = () => {
  return (
    <Content>
      <H5>What is Reapit’s Agency Cloud Desktop CRM?</H5>
      <p>
      ...
      <H5>Agency Cloud Interaction API</H5>
      <p>
      ...
      <Grid>
        <GridItem>
          <H5>Desktop Integration Types</H5>
          <p>
            To be able to associate an application with an action in Agency Cloud the application will need to be given
            a desktop type. This will be required so that Agency Cloud can be confident of the way the application will
            behave and that the application is agreeing to accept certain parameters when launched. These parameters
            will be available inside a global javascript object which is used to identify that a page is in{' '}
            <i>Desktop</i> mode, example below.
          </p>
          <Section>
            <code>{`window.__REAPIT_MARKETPLACE_GLOBALS__ = {
              prpCode: 'MK010201'
            }`}</code>
          </Section>
          <p>
            We currently support several application types and we are continuing to build as apps are developed. To show
            you an example, you can view the video below. It demonstrates an application that was built with a Desktop
            Type of &#39;Property&#39;, this means that the application will be available whenever a user interacts with
            a property:
          </p>
          <p>
