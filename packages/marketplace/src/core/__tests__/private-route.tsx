...
describe('PrivateRouter', () => {
  it('should match a snapshot', () => {
    expect(shallow(<PrivateRoute allow="CLIENT" component={() => null} />)).toMatchSnapshot()
  })

  it('should redirect to /404 page if isLogin is false', () => {
    const wrapper = mount(
      <MemoryRouter initialEntries={['/my-path']}>
        <PrivateRoute allow="CLIENT" component={() => null} path="/my-path" />
  ...
  it('should return render component if loginType matches allow is true', () => {
    const wrapper = mount(
      <MemoryRouter initialEntries={['/client']}>
        <PrivateRoute allow="CLIENT" component={() => <div className="render-class" />} path="/client" />
      </MemoryRouter>,
    )
