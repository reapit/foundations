...
export const fetchAppPipelineSaga = function* ({ data }: Action<FetchAppByIdParams>) {
  try {
...
export const Listen = function* () {
  yield takeLatest<Action<FetchAppByIdParams>>(ActionTypes.FETCH_APP_DETAIL, fetchAppPipelineSaga)
}

const appPipelineSagas = function* () {
  yield all([fork(Listen)])
}

export default appPipelineSagas
