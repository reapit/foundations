...
export interface DeveloperAppTrafficProps {
  // https://reapitgroup.slack.com/archives/CT5MHMZFE/p1594365897003000
  // UsageStatsModel is deprecated, not sure if this model is still used
  // TODO: investigate
  stats: any
...
export const generateUsageStatsData = ({ apps, stats }: DeveloperAppTrafficProps) => () => {
  return apps.data?.reduce<AppUsageStats[]>((prev, app) => {
    // AppUsageStatsModel is deprecated
    const appUsage = stats.appUsage?.find((item: any) => item.appId === app.id)
    const result = {
      appName: app.name,
      created: app.created,
