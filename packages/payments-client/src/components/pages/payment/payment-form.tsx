...
export const PaymentForm: FC = () => {
  ...
  const { paymentWithProperty, paymentProvider, paymentParams } = paymentsDataState
  const [paymentStatus, setPaymentStatus] = useState<PaymentStatusType>(
    paymentWithProperty?.status as PaymentStatusType,
  )
  const { error } = useSnack()
  const { forename = '', surname = '', email = '', primaryAddress } = paymentWithProperty?.customer ?? {}
  const merchantKey = paymentProvider?.merchantKey
  const { paymentId, session } = paymentParams
    ...
    <>
      ...
      {paymentStatus === 'posted' && (
        <>
          ...
          <ButtonGroup>{paymentWithProperty && <ResendConfirmButton paymentFormValues={getValues()} />}</ButtonGroup>
        </>
      )}
      {paymentStatus !== 'posted' && paymentWithProperty && paymentId && merchantKey && (
        <form
          onSubmit={handleSubmit(
            onHandleSubmit(merchantKey, paymentWithProperty, paymentId, setPaymentStatus, error, session),
          )}
        >
          ...
          <ButtonGroup>
            <Button intent="primary" type="submit" loading={isLoading} disabled={isLoading}>
              Make Payment
            </Button>
          </ButtonGroup>
        </form>
