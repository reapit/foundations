...
export const resolveArgs = (incomingArgs: string[], args?: ArgsType[]): any[] => {
  ...
  argTypes?.filter(argOptions => argOptions.paramOrOption.type === 'parameter').forEach(({paramOrOption: param, index: depIndex}, index) => {
  ...
  argTypes?.filter(argOptions => argOptions.paramOrOption.type === 'option').forEach(({paramOrOption: option, index}) => {
