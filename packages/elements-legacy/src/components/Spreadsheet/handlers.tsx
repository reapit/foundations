...
export const onDoubleClickCell =
  (payload: DoubleClickPayLoad, setSelected: SetSelected, onDoubleClickDefault) =>
  (...args): boolean => {
...
export const customCellRenderer =
  (data: Cell[][], setData: SetData, setSelected: SetSelected, afterCellsChanged?: AfterCellsChanged) =>
...
export const handleAddNewRow =
  (data: Cell[][], setData: SetData, allowOnlyOneValidationErrorPerRow: boolean, validate?: ValidateFunction) => () => {
  ...
  async (event: { target: HTMLInputElement }) => {
    try {
      ...
      if (target && target.files && target.files[0]) {
        ...
        const { dataWithInvalidRowsRemoved, invalidIndies } = createDataWithInvalidRowsRemoved(
          slicedData,
          validateMatrix,
        )
...
export const handleAfterDataChanged =
  (data: Cell[][], prevData?: Cell[][], afterDataChanged?: AfterDataChanged) => () => {
