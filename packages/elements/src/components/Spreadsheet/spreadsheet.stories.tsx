...
export const SpreadsheetOnly: Story = () => (
  <PortalProvider>
    <Spreadsheet data={data} hasAddButton={false} hasUploadButton={false} hasDownloadButton={false} />
  </PortalProvider>
)

export const SpreadsheetWithAfterDataChangedHandler: Story = () => (
  <PortalProvider>
    <Spreadsheet
      data={data}
...
export const CustomCompSpreadsheetWithAfterCellsChangedHandleronent: Story = () => {
  // eslint-disable-next-line
  const emailRegex = /^(([^<>()\[\]\.,;:\s@\"]+(\.[^<>()\[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i
...
export const UploadWithValidate: Story = () => {
  // eslint-disable-next-line
  const emailRegex = /^(([^<>()\[\]\.,;:\s@\"]+(\.[^<>()\[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i
  return (
    <PortalProvider>
      <Spreadsheet
        ...
        description={
          <p>
            ...
            <br />
            It is up to you to handle after uploading. The<code>afterUploadDataValidated</code> function will be called
            after finish validating uploaded data
          </p>
        }
      />
    </PortalProvider>
