...
<RenderHtmlMarkup component="Nav" story={['Basic Example']} label="Nav Rendered HTML markup" />
...
<Canvas>
  <Story name="Basic Example With Mobile Menu">
    <Nav>
      <NavItem>
        <Icon iconSize="medium" className={elNavItemIcon} icon="reapitLogoMenu" />
        <Icon className={cx(elIntentNeutral, elMLAuto, elMr2, elNavItemHideDesktop)} icon="hamburgerMenu" />
      </NavItem>
...
<RenderHtmlMarkup component="Nav" story={['Desktop Mode']} label="Nav with Mobile Menu Rendered HTML markup" />
...
export const UseNavMobileSubMenuStory = () => {
  ...
  return (
    <Nav>
      ...
      >
        <Icon className={elNavItemIcon} icon={isMobile ? 'reapitLogoTextMenu' : 'reapitLogoMenu'} />
        <Icon
          className={cx(elIntentNeutral, elMLAuto, elMr2, elNavItemHideDesktop)}
          icon={navMenuOpen ? 'hamburgerOpen' : 'hamburger'}
          onClick={setNavState({
