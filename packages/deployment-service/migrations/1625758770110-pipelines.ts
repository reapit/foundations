...
export class pipelines1625758770110 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query("CREATE TABLE `pipeline_runners` (`id` varchar(36) NOT NULL, `created` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `modified` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), `buildStatus` enum ('failed', 'success', 'canceled', 'pending', 'running') NOT NULL DEFAULT 'pending', `S3Location` varchar(255) NULL, `pipelineId` varchar(36) NULL, PRIMARY KEY (`id`)) ENGINE=InnoDB");
