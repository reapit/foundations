...
const fileName = (developerId: string, project: string, version: string): string => ([developerId, project, version].join(fileSeparator)) + '.zip'
...
export const releases = httpHandler<SimpleDeployDto, any[]>({
  ...
  handler: async ({ event, body }) => {
    ...
    const files = await new Promise<Object[]>((resolve, reject) => s3Client.listObjectsV2({
      Bucket: '',
...
export const releaseVersion = httpHandler({
  handler: async ({ event }) => {
    ...
    const file = await new Promise<AWS.S3.Body>((resolve, reject) => s3Client.getObject({
      Bucket: '',
