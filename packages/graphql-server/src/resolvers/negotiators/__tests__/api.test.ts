  ...
  describe('callUpdateNegotiatorAPI', () => {
    {
      it('should run correctly', async () => {
        mockedFetcher.mockImplementation(() => negotiatorStub)
        const updateNegotiatorArgs: UpdateNegotiatorArgs = { id: 'MLD', model: updateArgStub }
        const result = await callUpdateNegotiatorAPI(updateNegotiatorArgs, mockContext)

        expect(fetcher).nthCalledWith(1, {
          url: `${URLS.negotiators}/${updateNegotiatorArgs.id}`,
          api: REAPIT_API_BASE_URL,
          body: updateNegotiatorArgs.model,
          method: 'PATCH',
          headers: {
            Authorization: mockContext.authorization,
            'Content-Type': 'application/json',
      ...
      it('should handle error case correctly', async () => {
        ...
        const updateNegotiatorArgs = { id: 'MLD', model: updateArgStub }
        const result = await callUpdateNegotiatorAPI(updateNegotiatorArgs, mockContext)
