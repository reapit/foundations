...
export const selectSubsByAppId =
  (appId?: string) =>
  (state: ReduxState): SubscriptionModel[] => {
    const subscriptionsDefault: SubscriptionModel[] = []
    if (!appId) return subscriptionsDefault
    return (
      state.subscriptions.byDevAndType.subscriptions.filter((sub) => sub.applicationId === appId) ??
      subscriptionsDefault
    )
  }

export const selectSubsByDeveloperId =
  (developerId: string) =>
  (state: ReduxState): SubscriptionModel[] => {
    const subscriptionsDefault: SubscriptionModel[] = []
    return (
